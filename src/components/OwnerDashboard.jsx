...
export default function OwnerDashboard() {
  ...
  const calculateMetrics = (txList) => {
    ...
    txList.forEach(t => {
      ...
      if (t.paymentMethod === 'M-Pesa') mpesaTotal += t.grandTotal;
      else if (t.paymentMethod === 'Cash') cashTotal += t.grandTotal;
      else if (t.paymentMethod === 'Card') cardTotal += t.grandTotal;
    });
  ...
  // CSV Export
  const exportCSV = () => {
    const headers = ['ID,Timestamp,Client,Services,PaymentMethod,ServiceSubtotal,Barber,BarberTip,BarberTipPaid,MassageTherapist,MassageTip,MassageTipPaid,GrandTotal'];
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 space-y-6 pb-24">
      ...
      <div className="flex items-center bg-slate-900/90 p-1.5 rounded-2xl border border-slate-800 gap-1 overflow-x-auto">
        {[
          ...
          <button
            ...
            className={`flex-1 min-w-[130px] sm:min-w-0 py-2.5 px-3 rounded-xl text-xs font-bold transition-all text-center whitespace-nowrap ${
              activeTab === tab.id
